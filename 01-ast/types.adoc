...
`ast.Stmt` インタフェースはGoソースコードにおける https://go.dev/ref/spec#Statements[文] に対応する構文木のノードを表します。文はプログラムの実行を制御するもので、`go/ast` パッケージの実装では以下のように分類されています：
...
`ast.Expr` インタフェースはおもにGoソースコードにおける https://go.dev/ref/spec#Expressions[式] および https://go.dev/ref/spec#Types[型] に対応する構文木のノードを表します。`go/ast` パッケージの実装では以下のように分類されています：
...
* 関数リテラル（`ast.FuncLit`）
* https://go.dev/ref/spec#Composite_literals[複合リテラル]（`ast.CompositeLit`）
* 括弧（`ast.ParenExpr`）
